...
router.post("/updateclosed", async (req, res) => {
  ...
  for (let j = 0; j < hofAll.length; j++) {
    ...
    if (loc >= 0) {
      ...
      //hofAll = improvement.improvementCalc(currentLeague, pastLeague, hofAll, loc);

      //these have to be deleted so that they don't overwrite the data
      //WHAT is this why does hofAll need you to delete the fields for league? what have i done
